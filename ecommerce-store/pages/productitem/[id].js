...
const id = () => {
  ...
  const [productid, setProductid] = useState(productItem);
  ...
  const getProduct = async () => {
    setLoading(true)
    const response = await axios.post("/api/cart", { ids: productid });
  ...
  return (
    <>
        <Navbar/>
        {loading ? <Loader/>:
        <section className="bg-gray-200 pt-12 pb-24 bg-blueGray-100 rounded-b-10xl overflow-hidden">
          <div className="container px-4 mx-auto">
            <div className="flex flex-wrap -mx-4">
                  ...
                  <div className="w-full sm:w-auto min-w-max px-4 text-center flex sm:flex-col items-center justify-center">
                    ...
                    </Link>
                    {
                        products[0]?.images.map((image)=>{
                            return <Link className="h-30 block mb-4 mr-2 sm:mr-0" href="#">
                            <img
                              className="h-10 w-10"
                              src={image}
                              alt="img2"
              ...
              <div className="w-[80%] lg:w-1/2 px-4">
                <div className="max-w-md mb-6">
                  <span className="text-xs text-gray-400 tracking-wider">
                    {products[0]?.title} #3299803
                  </span>
                  <h2 className="mt-6 mb-4 text-3xl md:text-4xl lg:text-5xl font-heading font-medium">
                    {products[0]?.title} - Silver
                  </h2>
                ...
                <div className="flex flex-wrap -mx-2 mb-12">
                  <div className="w-full md:w-2/3 px-2 mb-2 md:mb-0">
                    <button
                      className="block py-4  leading-8 font-heading font-medium tracking-tighter text-xl text-white text-center bg-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 hover:bg-blue-600 rounded-xl px-20"
                      href="#" onClick={(e)=>{addToCart(e, products[0]?._id)}}
                    >
                      Add to Cart
                    </button>
                  </div>
