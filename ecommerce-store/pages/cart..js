                ...
                products.length > 0 && products.map((prod)=>{
                    ...
                    <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                      <div className="mt-5 sm:mt-0">
                        ...
                        <p className="mt-1 text-xs text-gray-700">${prod.price}</p>
                        <p className="mt-1 text-xs text-gray-700">{prod.description}</p>
                      </div>
                      <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                        <div className="flex items-center border-gray-100">
                        <span onClick={(e) => addToCart(e, prod._id)} className="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50"> +</span>
                                    <input className="h-8 w-8 border bg-white text-center text-xs outline-none" type="number" value={findProductInCartId(prod._id)} min="1" />
                                    <span onClick={(e) => removeProdCart(e, prod._id)} className="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50"> - </span>
                                 
                        </div>
                        <div className="flex  items-center space-x-4">
                          <p className="text-sm">${prod.price}</p>
                          <button onClick={(e) => {removeProdCart(e,prod._id)}}>
